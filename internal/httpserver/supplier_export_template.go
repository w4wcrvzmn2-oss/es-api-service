...
func exportFieldCatalog() []exportField {
	return []exportField{
		// — служебное —
		{"global_sign", "Номер (EX)", "Глобальный номер заказа EX-####### (добавляется всегда)"},
		...
		{"comment", "Комментарий", "Комментарий к заказу. Теги: COMMENT, NOTE"},

		// — прайс-лист (источник) —
		{"price_list_id", "ID прайс-листа", "ID прайс-листа, из которого заказ. Теги: ID_PRICE_LIST"},
		...
		{"unit_price", "Цена", "Цена за единицу. Теги: PRICE, COST, цена"},
		{"alt_price", "Альт. цена", "Альтернативная цена (часто 0). Теги: ALT_PRICE"},
		{"vat_rate", "НДС %", "Ставка НДС (часто 0.0000). Теги: VAT_RATE, НДС"},
		...
		{"registry_price", "Реестровая цена", "Реестровая цена (ЖНВЛС). Теги: REGCOST, REGCOST"},
	}
}
